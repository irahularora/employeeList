...
export default function EmployeList(props) {
    ...
    return (
        <div className="con-1">
            <h1>Employee LIst</h1>
            <EmployeeModel refer={ref} model={modelEmployee} showAlert={props.showAlert} />
            <button type="button" className="btn1 btn-primary" onClick={() => createEmploye()}>
                Create New Employee
            </button>
                        ...
                        {employees.map((employee, index) => {
                            ...
                            return (
                                <tr key={index}>
                                    ...
                                    <td style={{ textTransform: "uppercase" }}>{employee.gender}</td>
                                    <td>{new Date(employee.dob).toLocaleDateString('en-GB')}</td>
                                    <td>{employee.salary}</td>
                                    <td>{new Date(employee.joiningDate).toLocaleDateString('en-GB')}</td>
                                    <td>{employee.relievingDate ? new Date(employee.relievingDate).toLocaleDateString('en-GB') : 'NA'}</td>
                                    ...
                                    <td>
                                        <div className="action">
                                            ...
                                            <i
                                                ...
                                                onClick={() => deleteEmployee(employee._id)}                                            ></i>
                                        </div>
                                    </td>
                                </tr>
                            );
